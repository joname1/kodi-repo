...
#FCS_LIST = [['','首播'],['index-ml','大陆'],['index-ht','港台'],['index-us','欧美'],['index-kr','韩语'],['index-jp','日语'],['index-yyman','音悦人'],['index-elite','热门推荐']]
FCS_LIST = [['all','全部'],['ml','大陆'],['ht','港台'],['us','欧美'],['kr','韩国'],['jp','日本']]
MVR_LIST = [['all','全部推荐'],['ML','大陆推荐'],['HT','港台推荐'],['US','欧美推荐'],['KR','韩语推荐'],['JP','日语推荐']]
...
AREA_LIST = [['','全部地区'],['ML','大陆'],['HT','港台'],['US','欧美'],['KR','韩国'],['JP','日本']]
PAGE_LIST = [['1','TOP:1-20'],['2','TOP:21-40'],['3','TOP:41-50']]
VCHART_LIST = [['ML','大陆篇'],['HT','港台篇'],['US','欧美篇'],['KR','韩国篇'],['JP','日本篇']]
...
# area: 艺人地区       = 全部 大陆 港台 欧美 韩语 日语 其他
...
def playVideo(name,url,thumb):
    ...
    ret = pDialog.create('加载中..', '请耐心等待! 正在载入视频文件...')
    pDialog.update(0)
    ...
    for x in range(psize):
        ...
        if (errcnt >= ERR_MAX and k == 0):
            pDialog.close() 
            dialog = xbmcgui.Dialog()
            ok = dialog.ok(__addonname__, '无法播放：未找到该视频文件，请选择其它视频')
            break 
...
def playVideoX(name,url,thumb):
    ...
    if v_url:
        ...
        elif link.find('载入失败，请确认视频是否被删除')>0:
            dialog = xbmcgui.Dialog()
            ok = dialog.ok(__addonname__, '无法播放：该视频为收费节目')
